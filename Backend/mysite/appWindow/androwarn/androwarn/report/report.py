...
def filter_analysis_results(data, verbosity) :
	...
	'''
	{
		...
		'analysis_results' :
					...
					...................... For remaining part -> analysis.py perform_analysis()
	
	}
				
	'''

	# Iterate list of dicts and merge into 1 dict
	for itm in data:
		# Replace any '.' chars as MongoDB doesnt allow it in a key
		itm_key = itm.keys()[0]
		itm_key = itm_key.replace('.','_')
		itm_value = itm.values()[0]
		inner_itm = {}
		# Iterate inner list
		for iitm in itm_value:
			inner_itm[iitm[0]] = iitm[1]
		refined_data[itm_key] = inner_itm
	
	# Add search count (default = 0)
	refined_data['search_count'] = 0

	# Avoid storing duplicates
	if collection.find_one( { "application_information.package_name" : refined_data["application_information"]["package_name"] } ) == None:
		print 'hi'
		collection.insert_one(refined_data)
	
	# End of Nishad's code
